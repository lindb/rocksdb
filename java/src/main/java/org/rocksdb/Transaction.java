  ...
  */
  public Snapshot getSnapshot() {
    ...
    if(snapshotNativeHandle == 0) {
      ...
      snapshot.disOwnNativeHandle(); //TODO(AR) are the ownership semantics correct here?
      return snapshot;
    }
  }
  ...
  public byte[] getForUpdate(final ReadOptions readOptions,
    ...
    return getForUpdate(nativeHandle_, readOptions.nativeHandle_, key,
        key.length, columnFamilyHandle.nativeHandle_);
  }
  ...
  public WriteBatch getCommitTimeWriteBatch() {
    ...
    writeBatch.disOwnNativeHandle(); //TODO(AR) are the ownership semantics correct here?
    return writeBatch;
  }
  ...
  public ExecutionStatus getExecutionStatus() {
    ...
    return ExecutionStatus.getExecutionStatus(
        getExecutionStatus(nativeHandle_));
  }
  ...
  public enum ExecutionStatus {
    ...
    ExecutionStatus(final byte value) {
      this.value = value;
    }

    /**
     * Get ExecutionStatus by byte value.
     *
     * @param value byte representation of ExecutionStatus.
     *
     * @return {@link org.rocksdb.Transaction.ExecutionStatus} instance or null.
     * @throws java.lang.IllegalArgumentException if an invalid
     *     value is provided.
     */
    public static ExecutionStatus getExecutionStatus(final byte value) {
      for (final ExecutionStatus executionStatus : ExecutionStatus.values()) {
        if (executionStatus.value == value){
          return executionStatus;
        }
      }
      throw new IllegalArgumentException(
          "Illegal value provided for ExecutionStatus.");
    }
  }
  ...
  private native byte getExecutionStatus(final long handle);
