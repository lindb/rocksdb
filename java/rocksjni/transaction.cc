...
void Java_org_rocksdb_Transaction_setSnapshotOnNextOperation__JJ(JNIEnv* env,
    ...
    auto* txn = reinterpret_cast<rocksdb::Transaction*>(jhandle);
    // TODO(AR) this uses a ptr to a shared_ptr has the same issues as other such
    // uses in Java
    auto* txn_notifier =
...
jobjectArray txn_multi_get_helper(JNIEnv* env, const FnMultiGet &fn_multi_get,
    ...
    jsize len_key_parts = env->GetArrayLength(jkey_parts);
    if (env->EnsureLocalCapacity(len_key_parts) != 0) {
        // out of memory
        return NULL;
    }
    for (int i = 0; i < len_key_parts; i++) {
        jobject jk = env->GetObjectArrayElement(jkey_parts, i);
...
jobjectArray Java_org_rocksdb_Transaction_multiGet__JJ_3_3B_3J(JNIEnv* env,
    ...
    const std::vector<rocksdb::ColumnFamilyHandle*> column_family_handles =
            txn_column_families_helper(env, jcolumn_family_handles);
    auto* txn = reinterpret_cast<rocksdb::Transaction*>(jhandle);
 ...
 * Signature: (JJ[BIJ)[B
 */
jbyteArray Java_org_rocksdb_Transaction_getForUpdate__JJ_3BIJ(JNIEnv* env,
 ...
 * Signature: (JJ[BI)[B
 */
jbyteArray Java_org_rocksdb_Transaction_getForUpdate__JJ_3BI(JNIEnv* env,
                                                             jobject jobj, jlong jhandle, jlong jread_options_handle, jbyteArray jkey,
                                                             jint jkey_part_len) {
    auto* txn = reinterpret_cast<rocksdb::Transaction*>(jhandle);
    FnGet fn_get_for_update =
            std::bind<rocksdb::Status (rocksdb::Transaction::*) (const rocksdb::ReadOptions&, const rocksdb::Slice&, std::string*)>(
                    &rocksdb::Transaction::GetForUpdate, txn, _1, _2, _3);
    return txn_get_helper(env, fn_get_for_update, jread_options_handle, jkey,
                          jkey_part_len);
}
...
void txn_write_kv_parts_helper(JNIEnv* env,
    ...
    rocksdb::Slice* key_parts = new rocksdb::Slice[jkey_parts_len];
    rocksdb::Slice* value_parts = new rocksdb::Slice[jvalue_parts_len];

    std::tuple<jbyteArray, jbyte*, jobject>* jkey_parts_to_free =
            new std::tuple<jbyteArray, jbyte*, jobject>[jkey_parts_len];
    std::tuple<jbyteArray, jbyte*, jobject>* jvalue_parts_to_free =
            new std::tuple<jbyteArray, jbyte*, jobject>[jvalue_parts_len];

    // convert java key_parts/value_parts byte[][] to Slice(s)
    for (jint i = 0; i < jkey_parts_len; ++i) {
        jobject jobj_key_part = env->GetObjectArrayElement(jkey_parts, i);
        jobject jobj_value_part = env->GetObjectArrayElement(jvalue_parts, i);

        jbyteArray jba_key_part = reinterpret_cast<jbyteArray>(jobj_key_part);
        jbyteArray jba_value_part = reinterpret_cast<jbyteArray>(jobj_value_part);
        jsize jkey_part_len = env->GetArrayLength(jba_key_part);
        jsize jvalue_part_len = env->GetArrayLength(jba_value_part);
    ...
    for (jint i = jkey_parts_len - 1; i >= 0; --i) {
    ...
    delete[] jvalue_parts_to_free;
    delete[] jkey_parts_to_free;
...
void txn_write_k_parts_helper(JNIEnv* env,
    ...
    for (jint i = 0; i < jkey_parts_len; ++i) {
        ...
        jbyte* jkey = env->GetByteArrayElements(jba_key_part, nullptr);

        jkey_parts_to_free[i] = std::tuple<jbyteArray, jbyte*, jobject>(
                jba_key_part, jkey, jobj_key_part);

        key_parts[i] = rocksdb::Slice(reinterpret_cast<char*>(jkey), jkey_part_len);
    }
...
jbyte Java_org_rocksdb_Transaction_getExecutionStatus(JNIEnv* env,
                                                      jobject jobj, jlong jhandle) {
    auto* txn = reinterpret_cast<rocksdb::Transaction*>(jhandle);
    rocksdb::Transaction::ExecutionStatus exec_status = txn->exec_status_;
    switch (exec_status) {
